...
export async function deleteIfRecordsTodayExist(progress: JobProgress, graphUri: string, classType: string) {
    const dateStr = DateOnly.today().toString();
    const checkQuery = new TemplatedSelect<{ prefixes: string; date: string; graphUri: string, classType: string }, { report: string }>(
        queryEngine,
        config.env.REPORT_ENDPOINT,
        checkTodayRecordsTemplate
    );
    const existingReports = await checkQuery.records({ prefixes: PREFIXES, date: dateStr, graphUri: graphUri, classType: classType });
    if (existingReports.length > 0) {
        progress.update(`Found ${existingReports.length} reports for today. Deleting...`);

        const deleteQuery = new TemplatedInsert<{ prefixes: string; date: string; graphUri: string, classType: string }>(
            queryEngine,
            config.env.REPORT_ENDPOINT,
            deleteTodayRecordsTemplate
        );
        await deleteQuery.execute({ prefixes: PREFIXES, date: dateStr, graphUri: graphUri, classType: classType });
